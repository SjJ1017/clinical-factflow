...
Explicitly authorized GPU sharing is available with `ALLOW_GPU_SHARING=1`. This only relaxes the existing-process and idle-utilization checks; UUID selection, kernel verification and at least 38,000 MiB free memory are still enforced. Without this opt-in, launches require an idle GPU. Use a separate tmux name and output path when relocating a not-yet-started case shard, and never run duplicate copies of that shard.
